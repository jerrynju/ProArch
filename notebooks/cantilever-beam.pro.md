...
view_hints:
  ...
  beam-compute:
    calc:
      ...
      title: 挠度计算
      icon: calc
      aside: { label: 限值 L/250, symbol: allowable_mm, unit: mm }
  beam-plot:
    calc: { title: 挠度曲线, icon: plot }
  beam-verify:
    calc: { title: 规范校核, icon: check }
  beam-material:
    calc: { title: 材料应力校核, icon: check }
  beam-table:
    calc: { title: 荷载工况对照表, icon: table, placeholder: true }
  beam-sim:
    calc: { title: 动态响应仿真, icon: wave, placeholder: true }
---
